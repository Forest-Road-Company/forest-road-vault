#!/usr/bin/env python3
"""Compile two incorrect ledger-replacement guards against the runtime access-control inventory."""
...
stage = Path(tempfile.mkdtemp(prefix='frv-ledger-access-controls-'))
...
target = stage / 'src/DefaultManager.sol'
original = target.read_text()
before = 'function replaceCommitmentLedger() external onlyRole(DEFAULT_ADMIN_ROLE) accrualIdle {'
assert original.count(before) == 1
cases = [
    ('replacement-guard-removed', 'function replaceCommitmentLedger() external accrualIdle {'),
    ('replacement-wrong-role', 'function replaceCommitmentLedger() external onlyRole(Roles.GUARDIAN_ROLE) accrualIdle {'),
]
...
def run(label, positive):
    ...
    if positive:
        assert '7 tests passed, 0 failed, 0 skipped' in result.stdout, label
...
try:
    ...
    names = ['invariant_acl_probeTableIsTheWholeEnumeratedSurface',
             ...
             'test_acl_ledgerReplacementIsEnumeratedAndAuthorized']
    for name in names:
    ...
    target.write_text(original)
    assert target.read_bytes() == (source / 'src/DefaultManager.sol').read_bytes()
