...
/// @notice Allocates a proved sub-native-unit contractual discrepancy through Ethereum's native three-layer loss order.
...
library ReserveRoundingLib {
    /// @dev One in-memory allocation; no independent accounting or storage namespace.
    struct Allocation {
        uint256 prepaid;
        uint256 curator;
        uint256 backstop;
    ...
    function prepare(ReserveManager.ReserveStorage storage native, uint256 id) public {
        ...
        ReserveAccrualLib.prepareNativeLoss(native);
    ...
    /// @notice Uses prepaid marks, this facility's curator class, then sGROVE and clamped senior assets.
    function allocate(ReserveManager.ReserveStorage storage native, AccrualLoans.LifecycleWork memory work) public {
        ReserveAccrualStorageLib.State storage s = ReserveAccrualStorageLib.state();
        ...
        allocation.mark = native.principalImpairment[work.facilityId];
        if (allocation.mark > loss) allocation.mark = loss;
        allocation.prepaid = native.exitPrepaidAbsorption;
        if (allocation.prepaid > allocation.mark) allocation.prepaid = allocation.mark;
        if (allocation.prepaid != 0) {
            native.exitPrepaidAbsorption -= allocation.prepaid;
            emit IReserveManager.ExitPrepaymentConsumed(
                work.facilityId, allocation.prepaid, native.exitPrepaidAbsorption
            );
        }
        s.rounding.facilityId = work.facilityId;
        s.rounding.closureNonce = work.closureNonce;
        s.rounding.active = true;
        allocation.curator = _curator(s, native, work.facilityId, loss - allocation.prepaid);
        allocation.backstop = _backstop(s, native, work.facilityId, loss - allocation.prepaid - allocation.curator);
        if (allocation.curator + allocation.backstop != 0) {
            _burn(s, address(this), allocation.curator + allocation.backstop);
        }
        allocation.senior = loss - allocation.prepaid - allocation.curator - allocation.backstop;
        uint256 available = IsUSDfr(s.modules.vault).totalAssets();
        if (allocation.senior > available) allocation.senior = available;
        if (allocation.senior != 0) _burn(s, s.modules.vault, allocation.senior);
        allocation.unabsorbed = loss - allocation.prepaid - allocation.curator - allocation.backstop - allocation.senior;
        s.roundingUnabsorbed += allocation.unabsorbed;
        ReserveCreditLib.writeDown(native, work.facilityId, loss);
        s.recordedFace -= loss;
        _notify(s, work.facilityId, loss);
        uint256[5] memory after_ = _measure(s, native);
        _equal(0, before_[0] - allocation.curator - allocation.backstop - allocation.senior, after_[0]);
        _equal(1, before_[1], after_[1]);
        _equal(2, before_[2] - allocation.senior, after_[2]);
        uint256 reduction = loss - allocation.mark;
    ...
    /// @dev sGROVE's recorded executable reserve is drawn only after this class's curator capital.
    function _backstop(
        ReserveAccrualStorageLib.State storage s,
        ReserveManager.ReserveStorage storage native,
        uint256 id,
        ...
        if (loss == 0) return 0;
        ICascadeBackstop backstop = native.lossBackstop;
        uint256 expected = backstop.remainingCoverage(id);
        if (expected > loss) expected = loss;
        uint256 balance = IERC20(s.modules.token).balanceOf(address(this));
        covered = backstop.coverShortfall{gas: _forwardGas()}(id, loss);
        _equal(8, expected, covered);
        _equal(9, balance + covered, IERC20(s.modules.token).balanceOf(address(this)));
    }
    ...
    function _measure(ReserveAccrualStorageLib.State storage s, ReserveManager.ReserveStorage storage native)
        private
        view
