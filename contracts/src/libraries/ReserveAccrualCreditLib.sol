...
library ReserveAccrualCreditLib {
    ...
    function register(ReserveManager.ReserveStorage storage native, uint256 id) public {
        ...
        address asset = address(native.usdcToken);
        ...
        uint256 scale = ReserveStorageLib.USDC_SCALE;
    ...
    function checkpoint(ReserveManager.ReserveStorage storage native, uint256 maximum)
        public
        ...
        for (uint256 i; i < processed; ++i) {
            ...
            if (item.stopped) {
                _replaceCeiling(
                    s,
                    item.facilityId,
                    native.deployed[item.facilityId] + ReserveAccrualStorageLib.facilityUnposted(item.facilityId)
                );
            }
    ...
    function post(ReserveManager.ReserveStorage storage native, uint256 id) public returns (uint256 amount) {
        ReserveAccrualStorageLib.State storage s = _fresh();
        _identity(s, id);
        s.busy = true;
        amount = s.loans.post(id, ReserveAccrualStorageLib.now64());
        _post(native, s, id, amount);
        s.busy = false;
    }
    ...
    function retire(ReserveManager.ReserveStorage storage native, uint256 id) public {
        ReserveAccrualStorageLib.State storage s = _fresh();
    ...
    function amend(ReserveManager.ReserveStorage storage native, uint256 id, IAccrualLifecycle.Terms memory terms)
        public
    {
        ReserveAccrualStorageLib.State storage s = _fresh();
    ...
    function stop(ReserveManager.ReserveStorage storage native, uint256 id) public {
        ReserveAccrualStorageLib.State storage s = _fresh();
        _caller(s.modules.defaultManager);
        _identity(s, id);
        ReserveRoundingLib.prepare(native, id);
        ...
        _aligned(native, s, work);
        _replaceCeiling(s, id, native.deployed[id]);
        s.busy = false;
    }
    ...
    function _aligned(
        ...
        _post(native, s, work.facilityId, work.posting);
        if (work.roundingLoss != 0) ReserveRoundingLib.allocate(native, work);
