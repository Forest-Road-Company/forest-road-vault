...
library DefaultAccrualLib {
    ...
    // Bounded below Ethereum's transaction gas cap, including marked loans and fee hooks.
    // Larger backlogs must use preparation batches before declaration.
    ...
    function bind(DefaultManager.DefaultStorage storage $, address reserve) public {
        ...
        _requireMatchingBackstop(reserve, address($.backstop));
        $.accrualReserve = IContinuousAccrual(reserve);
        emit DefaultAccrualBound(reserve);
    }

    /// @dev Recheck at the manager's binding boundary: its backstop can change after the
    ///      reserve configures its permanent routes but before this manager becomes bound.
    function _requireMatchingBackstop(address reserve, address backstop) private view {
        (bool ok, bytes memory reply) = reserve.staticcall(abi.encodeCall(IReserveManager.reserveLossModules, ()));
        if (!ok || reply.length != 160) revert DefaultAccrual_WrongModules();
        uint256[5] memory routes = abi.decode(reply, (uint256[5]));
        if (routes[1] != uint256(uint160(backstop))) revert DefaultAccrual_WrongModules();
    }
    ...
    ///      above 16 requires preparatory batches. Once enabled, native freshness and registration
    ...
    /// @notice Native ADR-0035 residual using five class pools and the one recorded sGROVE reserve.
    /// @dev Past-due consumes its class curator first. Declared claims consume the remainder;
    ///      the shared, uncapped backstop funds the remaining loss, with past-due taking priority.
    ///      No event-owned cap survives ADR-0035. Each class consumes min(total demand, pool),
    ///      regardless of event order, and the shared reserve consumes min(total residual, reserve).
    ///      The native ledger and both event orders remain the independent equivalence oracle.
    function nativeSeniorImpairment(DefaultManager.DefaultStorage storage $) public view returns (uint256) {
        uint256 residual;
        ...
        uint256 coverage = address($.backstop) == address(0) ? 0 : $.backstop.coverageReserve();
    ...
    function exactStateHash(DefaultManager.DefaultStorage storage $) public view returns (bytes32) {
        return keccak256(
            abi.encode(riskStateHash($), address($.backstop) == address(0) ? 0 : $.backstop.coverageCapacity())
        );
    }
    ...
    ///      the shared capacity is returned separately for its existing directional check.
        ...
        returns (bytes32 riskHash, uint256 exposure, uint256 capacity)
    {
        ...
        capacity = address($.backstop) == address(0) ? 0 : $.backstop.coverageCapacity();
    }
