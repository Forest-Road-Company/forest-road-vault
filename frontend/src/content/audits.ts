...
export const AUDITS: AuditReport[] = [
  {
    ...
    scope:
      "The whole protocol: 37 Solidity files, 10,502 lines of contracts/src. The reviewed commit is byte-identical to the current contracts/src tree at b5245398. First review of this protocol conducted by a party other than Forest Road.",
    method:
  ...
  },
  {
    ...
    findings: [
      ...
      },
      {
        ...
        note:
          "Before remediation, standalone attestation simulation, executor simulation and gas estimation all sent the complete signed bundle to READ_RPC_URL. Because recording a mark is permissionless, that bundle was a bearer credential: the endpoint operator could consume it outside the executor and split the intended atomic operation. RATED LOW because exploitation requires control or compromise of configured infrastructure, the obvious exit-ahead-of-loss impact fails against the queue's cooldown and settlement-time pricing, and the guardian path fails closed. FIXED 4 AUGUST 2026: the exact EIP-712 digest and quorum authorization are derived locally; peer recovery uses facility-filtered already-public executor events rather than a potentially dictionary-attackable pre-inclusion digest query; acceptance matches the oracle's canonical 65-byte low-s v=27/28 rule; a required fixed EXECUTION_GAS_LIMIT replaces dynamic estimation; and no digest commitment, unsigned valuation or signed attest/execute calldata reaches either RPC. A remediation-lens pass then measured the gas cost of blind submission after simulation removal; the worker now mirrors the executor action from a block-consistent signature-free snapshot and rejects paused, stale, non-live or no-action states before release. The compiled pinned-mainnet-fork lifecycle passes with rpc-commitment-leak=0, rpc-valuation-leak=0, rpc-bearer-leak=0 and zero relay calls for its no-action case. The approved confidential relay is now the first external bearer recipient; provider confidentiality, private revert suppression and final gas-limit approval remain production evidence gates, not open code findings.",
      },
      {
        id: "MTM-LIVENESS",
        severity: "Informational",
  ...
  },
  {
    ...
    findings: [
      ...
      },
      {
        ...
        disposition: "Open",
        note:
          "This work introduced a role that authorises the fee-neutral bracket around junior-capacity writes, and wired it correctly — granted to exactly three module contracts, asserted both positively and negatively in post-deploy validation. But it is missing from the generic privilege enumerator and from the handover script's role sets, whose own documentation states they mirror the roles library. The consequence is narrow and specific: targeted validation covers the role, so a misgrant would be caught, but the durable handover receipt cannot name it, so the artifact retained as evidence of who held what at genesis is silently incomplete.",
      },
    ],
  },
  {
