...
differences between the pre-mainnet release branch and the mainline it will merge into —
252 files across the contracts, the deployment and validation scripts, and the frontend.
...
alternative of silently rolling the payment date forward on activation should be rejected —
it would let an operator rewrite an attested economic term outside the amendment quorum.
