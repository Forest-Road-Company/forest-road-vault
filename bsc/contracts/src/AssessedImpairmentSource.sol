...
///         absolute USDfr loss AFTER applying estimated recoveries and the cascade - which on this
///         instance is curator first-loss and nothing after it (ADR-0037 D3a(ii)); this avoids the
///         incorrect result produced by simply multiplying the zero-recovery senior impairment by
...
///           past-due mark, recovery, realization or curator-capacity change invalidates it
///           immediately and restores the conservative base;
///         - the directional capacity check remains part of the shared assessment interface.
///           This instance returns zero capacity because it has no layer-two backstop;
...
contract AssessedImpairmentSource is Initializable, AccessControlUpgradeable, UUPSUpgradeable, IImpairmentSource {
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        // curator-capacity changes cannot manufacture performance.
        ...
        // Always zero on this instance (C-08): the source has no layer two to report capacity for.
        // Snapshotting it anyway keeps `_assessmentStateMatches`'s conjunction intact and keeps the
        // two instances' assessment lifecycles identical.
    ...
    /// @inheritdoc IImpairmentSource
    /// @dev The assessment snapshots the junior-capital credit standing at publication, so a
    ///      later change in junior capacity cannot move this fee base even where it improves
    ///      redemption protection. New overdue income is fully reserved in both assessment
    ///      views. Expired, invalid, cleared or pre-upgrade assessments fail
    ///      conservatively to the base source's gross view.
    ...
    /// @return matches Whether the live risk identity, capacity and exposure remain compatible.
    ///         Exact hashes may differ as overdue interest grows. Both assessed impairment
    ///         amounts reserve that growth; this instance still has zero layer-two capacity.
