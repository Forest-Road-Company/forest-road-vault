...
///      TWO kinds are excluded from level 2 on purpose, and both are OBSERVATION SERIES rather than
///      one-shot events: `Valuation`, and `ReserveAssetPrice` (ADR-0038). Each has its OWN
///      anti-rollback high-watermark, on its own mapping, so a stale-but-genuinely-signed
///      observation can never roll back a newer one and a revoked observation can never return.
///      `Valuation` is a monotone
    ...
    IAttestationOracle
{
    ...
    struct OracleStorage {
        ...
        // -- ADR-0038 (spec 06 section A.3, edit 4): reserve-asset price anti-rollback watermark --
        // The highest `asOf` ever ACCEPTED for one reserve-asset SUBJECT
        // (`uint256(uint160(asset))`), on its OWN clock. Deliberately not `valuationWatermarks`:
        // that mapping carries no kind term, so sharing it would let a facility valuation and an
        // asset price contend for one monotone clock on an id collision. Append-only TAIL.
        mapping(uint256 subject => uint64) assetPriceWatermarks;
    }
    ...
    /// @notice ADR-0038 (spec 06 section A.3): representability ceiling on an attested reserve price.
    /// @dev NOT AN ECONOMIC NUMBER, and it must not be read as one. It is twice par, which is far
    ///      outside any state in which a listed reserve stablecoin is still a reserve stablecoin.
    ///      A price ABOVE par is legal and is CAPPED ON READ at the mint edge, never refused here:
    ///      refusing it would stall the series in exactly the state where the asset is sound.
    ...
    function initialize(address admin, address guardian, address upgrader) external initializer {
        ...
        // Include the newly appended kind on fresh deployments. Existing proxies retain a
        // zero threshold for it until governance explicitly configures the required quorum.
        for (uint8 k = 0; k <= uint8(AttestationKind.AccrualOpening); ++k) {
            ...
            uint8 m = _isHighValueKind(kind) ? 2 : 1;
            $.thresholds[kind] = m;
    ...
    function attest(AttestationInput calldata a, bytes[] calldata signatures) external whenNotPaused {
        ...
        // `ReserveAssetPrice` IS DELIBERATELY EXCLUDED TOO (ADR-0038, spec 06 section A.3 edit 3), and the
        // argument is the Valuation argument again rather than an inherited one. A price series
        // REPEATS VALUES CONSTANTLY: an asset that trades at par across two consecutive observation
        // windows produces the identical payload. Left as a one-shot fact, the second push would
        // revert `Oracle_FactAlreadyRealised` and that asset's price could never be refreshed AT
        // THAT VALUE again - so it would age out under the reserve's staleness bound and close
        // minting in that asset permanently. Its uniqueness key is the `assetPriceWatermarks`
        // high-watermark enforced below, which makes both replay AND revocation durable.
        bool oneShot = a.kind != AttestationKind.Valuation && a.kind != AttestationKind.ReserveAssetPrice;
        bytes32 factKey_;
        if (oneShot) {
        ...
        } else if (a.kind == AttestationKind.ReserveAssetPrice) {
            // ADR-0038 / spec 06 section A.3 edit 4. `a.facilityId` is the SUBJECT here:
            // `uint256(uint160(reserveAsset))`. The reserve derives that subject itself when it
            // latches, so no relayer can point the reserve at another asset's record.
            uint256 p = uint256(a.payload);
            if (p == 0) revert Oracle_ZeroPrice();
            if (p > MAX_ATTESTED_PRICE) revert Oracle_PriceOutOfRange(p, MAX_ATTESTED_PRICE);
            // Same anti-rollback shape as H-02, on its OWN watermark, taking `max` with the live
            // record for the same in-place-upgrade degradation reason.
            uint64 priceFloor = $.assetPriceWatermarks[a.facilityId];
            if (r.asOf > priceFloor) priceFloor = r.asOf;
            if (a.asOf <= priceFloor) revert Oracle_StalePrice(a.asOf, priceFloor);
            $.assetPriceWatermarks[a.facilityId] = a.asOf;
        }
    ...
    function consume(uint256 facilityId, AttestationKind kind) external onlyRole(Roles.CREDIT_ROLE) {
        // ADR-0038 (spec 06 section A.3 edit 5) - DO NOT DELETE. `consume` is reachable by ANY wired
        // CREDIT_ROLE module for ANY kind, and it sets `satisfied = false`. On a reserve-asset
        // price that closes minting for that asset until the next push, which is a liveness break a
        // credit module has no business causing: no credit path spends a price.
        if (kind == AttestationKind.ReserveAssetPrice) revert Oracle_KindNotConsumable(kind);
        OracleStorage storage $ = _storage();
    ...
    function revoke(uint256 facilityId, AttestationKind kind) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (kind == AttestationKind.Valuation) {
            ...
            r.payload = bytes32(0);
            r.asOf = 0;
        } else if (kind == AttestationKind.ReserveAssetPrice) {
            // ADR-0038 (spec 06 section A.3 edit 6). MIRRORS THE VALUATION BRANCH AND WRITES NO TOMBSTONE.
            // A permanent fact tombstone on a price would kill ONE PRICE VALUE forever, and a price
            // series legitimately revisits values; the asset would then be unable to be re-marked
            // at that value and would age out. Seeding the watermark first makes the monotone clock
            // survive revocation on every path, exactly as H-02 does.
            //
            // WHAT REVOCATION THEREFORE DOES: the reserve's own latch is untouched by this call and
            // simply ages out under its staleness bound, closing mint for that asset. A strictly
            // newer honest push restores it.
    ...
    function setThreshold(AttestationKind kind, uint8 threshold_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (threshold_ < 2 && _isHighValueKind(kind)) revert Oracle_BadThreshold();
    ...
    /// @inheritdoc IAttestationOracle
    function assetPriceWatermark(uint256 subject) external view returns (uint64) {
        return _storage().assetPriceWatermarks[subject];
    }

    /// @inheritdoc IAttestationOracle
    function maxAttestedPrice() external pure returns (uint256) {
        return MAX_ATTESTED_PRICE;
    }
    ...
    /// @dev THE SINGLE ENUMERATION of the kinds that may never be lowered below 2-of-n, shared by
    ///      `initialize` and `setThreshold` so the default and the floor can never disagree.
    ///      Every one of them authorizes a VALUE-MOVING or state-freezing action, so no single
    ///      compromised attester key may act alone. `ReserveAssetPrice` joins the list under
    ///      ADR-0038: it authorizes a mint-side input, and spec 06 section 10(5) recommends Forest Road
    ///      set it ABOVE the documentary kinds. Documentary assignment/perfection kinds stay 1-of-n.
    function _isHighValueKind(AttestationKind kind) private pure returns (bool) {
        return kind == AttestationKind.CreditIssued || kind == AttestationKind.Valuation
            ...
            || kind == AttestationKind.TermsAmended || kind == AttestationKind.ReserveAssetPrice
            || kind == AttestationKind.AccrualOpening;
    }
