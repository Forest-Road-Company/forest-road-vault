...
import {IAccrualLifecycle, IAccrualBridge} from "./interfaces/IAccrualLifecycle.sol";
import {BridgeAccrualLib} from "./libraries/BridgeAccrualLib.sol";
...
///         lien-perfected facility. THE SYNCHRONIZED MINT GATE (CLAUDE.md section 1.3): a
...
///         conditioned off-chain on the existence of this token (legal wrapper section 3).
///
    ...
    IAccrualBridge
{
    ...
    struct Facility {
        ...
        LoanState state;
        /// @notice Whether this facility contractually CAPITALISES unpaid interest (PIK).
        /// @dev TAIL-APPENDED 2026-09-08. Set once at origination and never amended: `Amendment`
        ///      deliberately does not carry it, so a cash-pay facility can never be converted into a
        ///      capitalising one by a later signature. Converting the other way would also be a
        ///      restructuring, and a restructuring should be a new facility rather than a flag flip.
        ///
        ///      IT IS IN THE TERMS HASH, so it is signed by the `CreditIssued` quorum along with the
        ///      rate. Without it `WaterfallEngine.capitalizePik` had nothing to distinguish a PIK
        ///      facility from an ordinary cash-pay one, and any address could force PIK accounting
        ///      onto the cash-pay book: the interest was capitalised into `deployed` AND then paid
        ///      again in cash through `distribute`, leaving a phantom receivable that only a real
        ///      charge against curator first-loss could ever clear.
        bool pik;
    }
    ...
    struct OriginationTerms {
        ...
        bytes32 offchainRef;
        /// @notice Whether this facility contractually capitalises unpaid interest. Signed.
        bool pik;
    }
    ...
    struct BridgeStorage {
        ...
        // Permanent continuous source, appended after all historical bridge fields.
    ...
    /// @dev AUDIT FIX (section 3.1 reconstructability): carries the full originated terms so
    ...
    /// @notice Governance permanently bound this bridge's authenticated accrual source.
    ...
    /// @notice Legacy PIK coupon dates can change only after migration to the native lifecycle.
    error Bridge_LegacyPikScheduleRequiresMigration(uint256 tokenId);
    ...
    /// @notice The continuous accounting source cannot be replaced after binding.
    error Bridge_AccrualAlreadyBound();
    /// @notice Only the bound source can advance its signed PIK capitalization schedule.
    error Bridge_AccrualReserveOnly(address caller);
    /// @notice A live continuous facility must use the supported fixed-rate day-count scope.
    error Bridge_AccrualUnsupportedTerms(uint256 tokenId);
    /// @notice A continuous PIK schedule advances only through its accounting source or signed amendment.
    error Bridge_AccrualDueManaged(uint256 tokenId);
    ...
    /// @notice Permanently binds the reserve after the underlying token has bound that same source.
    /// @param reserve The already configured reserve proxy for this bridge's native module routes.
    function setAccrualReserve(address reserve) external onlyRole(DEFAULT_ADMIN_ROLE) {
        BridgeStorage storage $ = _storage();
        if ($.accrualReserve != address(0)) revert Bridge_AccrualAlreadyBound();
        BridgeAccrualLib.validate(reserve, address($.registry), address($.oracle));
        IAccrualExposure(reserve).requireAccrualIdle();
        $.accrualReserve = reserve;
        emit AccrualReserveSet(reserve);
    }

    /// @notice The permanent accounting source, or zero before continuous accrual is bound.
    ...
    ///        - an EMPTY mask: a zero gate makes the section 1.3 "NFT mint gate" invariant vacuous for
    ...
    function _originate(address holder, Facility memory f) private returns (uint256 tokenId) {
        ...
        if (
            ...
            // THE SCHEDULE MUST CONTAIN AT LEAST ONE WHOLE INTERVAL BEFORE ITS FIRST PAYMENT.
            // `WaterfallEngine.fund` anchors the PIK cursor at `nextPaymentDue - paymentInterval`
            // so the capitalisation clock and the payment schedule agree by construction; with
            // an interval longer than the first due date that subtraction underflows and `fund`
            // reverts with a bare `Panic(0x11)`, stranding a fully attested facility in `Pending`
            // with no diagnostic. The cursor write is unconditional, so a CASH-PAY facility was
            // bricked by it too. Rejecting the terms here fails closed at the earliest point
            // instead, and every legitimate schedule already satisfies it: `nextPaymentDue` is an
            // absolute timestamp and `paymentInterval` a duration.
            f.principal == 0 || f.borrowerId == bytes32(0) || f.fundingRecipient == address(0) || f.interestRateBps == 0
                ...
                || f.paymentInterval > f.nextPaymentDue || f.paymentScheduleHash == bytes32(0)
                || f.offchainRef == bytes32(0)
        ...
        // PIK ONLY SERVICES FIXED / ACTUAL-360, SO REFUSE THE COMBINATION AT THE DOOR.
        // `WaterfallEngine._planPik` reverts `Waterfall_PikRateTypeUnsupported` and
        // `Waterfall_PikDayCountUnsupported` for anything else, and `capitalizePik` is the ONLY
        // writer of `nextPaymentDue` for a PIK facility. Without this check a PIK facility could be
        // created, or amended into, a shape whose crank can never run: the schedule would stop
        // advancing immediately and the borrower would become permissionlessly markable past due,
        // for a term combination the PROTOCOL has not implemented rather than anything they did.
        //
        // `pik` is set once at origination and `Amendment` deliberately does not carry it, so the
        // two sides of this check can never be amended apart. Added 2026-09-10 after an adversarial
        // round found the resulting freeze as the SEVENTH member of the PIK liveness family; the
        // sheltering half of the fix is in `WaterfallEngine.pikCrankBlockedByProtocol`, and this is
        // the half that stops the state arising at all.
        if (f.pik && (f.rateType != RateType.Fixed || f.dayCountConvention != DayCountConvention.Actual360)) {
            revert Bridge_BadFacility();
        }
        // AND THE THIRD SCOPE CONDITION, WHICH THE FIRST VERSION OF THIS GATE MISSED. `_planPik`
        // enforces THREE things, not two: Fixed, Actual-360, and a RECEIVABLE class. Gating only
        // the first two let a PIK facility be originated and FUNDED into a marked-to-market class,
        // where it could then take interest in neither form: every `capitalizePik` reverts
        // `Waterfall_PikClassNotReceivable`, and `distribute` refuses any PIK facility carrying an
        // interest leg. A funded loan that can never earn or receive a wei, with nothing anywhere
        // saying so. `CLASS_DIGITAL_ASSETS` is MarkedToMarket and active on this instance too, so
        // this was reachable rather than theoretical, and it CANNOT be repaired afterwards: `pik`
        // is written once at origination and the class is not amendable. Found by an adversarial
        // round 2026-09-10, measured on a 500,000e18 digital-asset facility.
        if (f.pik && p.model != ICollateralRegistry.CollateralModel.Receivable) revert Bridge_BadFacility();
    ...
    function checkFundable(uint256 tokenId) external view {
        ...
        if (_accrualEnabled($)) _requireAccrualTerms(tokenId, f.pik, f.rateType, f.dayCountConvention);
    ...
    /// @notice Whether a credit operation can presently update the bridge's guarded schedule.
    /// @dev A temporary operation lock is not evidence that a borrower missed a payment.
    function creditOperationIdle() external view returns (bool) {
    ...
    function setNextPaymentDue(uint256 tokenId, uint64 nextDue) external onlyRole(Roles.CREDIT_ROLE) nonReentrant {
        ...
        if (f.pik && _accrualEnabled($)) revert Bridge_AccrualDueManaged(tokenId);
    ...
    /// @inheritdoc IAccrualBridge
    /// @dev This reserve-only continuation runs during bounded keeper accounting. It deliberately
    ///      has no source-idle or freshness gate; it cannot amend terms, invent a maturity extension,
    ///      alter cash due dates or accept the coordinator's terminal zero sentinel.
    function setAccruedPaymentDue(uint256 tokenId, uint64 nextDue) external {
        BridgeStorage storage $ = _storage();
        if ($.accrualReserve == address(0) || msg.sender != $.accrualReserve) {
            revert Bridge_AccrualReserveOnly(msg.sender);
        }
        Facility storage f = _facility($, tokenId);
        if (!f.pik) revert Bridge_BadFacility();
        if (f.state != LoanState.Active && f.state != LoanState.Amortizing) {
            revert Bridge_InvalidTransition(tokenId, f.state, f.state);
        }
        uint64 previous = f.nextPaymentDue;
        ...
        nonReentrant
    {
        ...
        if (
            amendmentId == bytes32(0) || a.interestRateBps == 0 || a.interestRateBps > MAX_INTEREST_RATE_BPS
                ...
                || a.paymentInterval > a.nextPaymentDue // same schedule sanity as origination
        ...
        // THE SAME GATE ON THE AMENDMENT PATH, and this is the one that actually mattered. An
        // amendment writes `rateType` and `dayCountConvention` freely (below), so without this a
        // servicer could amend a LIVE, PERFORMING PIK facility into a shape its crank cannot
        // service and freeze it permanently. See the note at origination.
        if (f.pik && (a.rateType != RateType.Fixed || a.dayCountConvention != DayCountConvention.Actual360)) {
            revert Bridge_BadFacility();
        }

        bool continuous = _accrualEnabled($);
        if (continuous) _requireAccrualTerms(tokenId, f.pik, a.rateType, a.dayCountConvention);
        // The legacy waterfall retains its original coupon cursor. Refuse an amendment
        // it cannot execute; the native lifecycle below can close and replace that schedule.
        if (!continuous && f.pik && (a.paymentInterval != f.paymentInterval || a.nextPaymentDue != f.nextPaymentDue)) {
            revert Bridge_LegacyPikScheduleRequiresMigration(tokenId);
        }
        ...
        // The reserve sees the old stored contract and a consumed exact attestation. It closes
        // the old curve at this block, keeps the frozen PIK basis and installs only future terms.
        // Any reserve failure atomically restores attestation consumption and every bridge field.
        if (continuous) {
            IAccrualLifecycle($.accrualReserve).amendAccruingLoan(
                tokenId,
                IAccrualLifecycle.Terms({
                    rateBps: a.interestRateBps,
                    yearSeconds: a.dayCountConvention == DayCountConvention.Actual360 ? uint32(360 days) : uint32(365 days),
        ...
        view
    {
        ...
        // An unset class must satisfy the same mandatory gate as a configured class.
        if (mask & BIT_CREDIT_ISSUED == 0) revert Bridge_BadAttestationMask(mask);
        ...
        // ORDER IS LOAD-BEARING BELOW THIS LINE. DO NOT REORDER, AND DO NOT NARROW THE LOOP ABOVE.
        //
        // Both remaining `latestPayload` reads in this function DISCARD the `ok` flag, and that is
        // safe ONLY because of what the loop above has already done: it reverts
        // `Bridge_AttestationMissing` for EVERY bit set in `mask`, over the same
        // `0..KIND_COUNT-1` range these two reads are gated on, and this function is `view` so the
        // oracle cannot change underneath them. Under those three conditions a dropped `ok` cannot
        // hide an absent attestation, because an absent one has already reverted.
        //
        // Reorder these blocks above the loop, narrow the loop's range or its mask, or make this
        // function non-view, and BOTH reads below become real defects: a missing attestation would
        // read as `payload == bytes32(0)` and be compared against `termsHash` rather than refused.
        // Recorded 2026-09-10 during the first Slither triage on this tree, which dispositioned
        // these two `unused-return` findings as safe on exactly this reasoning. A property that
        // holds by ordering has to be stated where someone would break it.
        //
    ...
    /// @dev Token approvals cannot change during a source accounting or delivery callback.
    ...
    /// @dev Operator approvals share the same callback boundary as ownership changes.
    function _setApprovalForAll(address owner, address operator, bool approved) internal override {
        _requireAccrualIdle();
        super._setApprovalForAll(owner, operator, approved);
    }

    /// @dev Includes initial grants before binding and governed grants afterwards.
    function _grantRole(bytes32 role, address account) internal override returns (bool) {
        _requireAccrualIdle();
        return super._grantRole(role, account);
    }

    /// @dev Includes revocation and a holder's own role renunciation.
    function _revokeRole(bytes32 role, address account) internal override returns (bool) {
        _requireAccrualIdle();
        return super._revokeRole(role, account);
    }

    /// @dev An upgrade cannot replace lifecycle accounting during a protected source operation.
    function _authorizeUpgrade(address) internal view override onlyRole(Roles.UPGRADER_ROLE) {
        _requireAccrualIdle();
    }

    /// @dev Admissions and prospective economic amendments require the authoritative current book.
    ...
    /// @dev Recovery, governance and custody changes need an idle source, independent of keeper lag.
    function _requireAccrualIdle() private view {
        address reserve = _storage().accrualReserve;
        if (reserve != address(0)) IAccrualExposure(reserve).requireAccrualIdle();
    }

    /// @dev Binding prepares modules before activation and does not rewrite legacy funded terms.
    function _accrualEnabled(BridgeStorage storage $) private view returns (bool) {
        return $.accrualReserve != address(0) && IContinuousAccrual($.accrualReserve).accrualSnapshot().enabled;
    }

    /// @dev Continuous cash supports fixed Actual/360 and Actual/365; PIK retains fixed Actual/360.
    function _requireAccrualTerms(uint256 tokenId, bool pik, RateType rateType, DayCountConvention dayCount)
        private
        pure
    {
        if (
            rateType != RateType.Fixed || dayCount == DayCountConvention.Thirty360
                || (pik && dayCount != DayCountConvention.Actual360)
        ) revert Bridge_AccrualUnsupportedTerms(tokenId);
    }
