...
import {ReserveBasketLib} from "./ReserveBasketLib.sol";
import {Math} from "@openzeppelin/contracts/utils/math/Math.sol";

/// @title ControllerAccrualLib
/// @notice Linked controller helpers for accrual accounting and recorded withdrawal admission.
/// @dev Executes in the controller's context. Its host owns governance and reentrancy guards.
library ControllerAccrualLib {
    /// @notice Reads payable reserves and enforces the holder's own deposit cap during review.
    /// @dev Pending reviews count even for empty or frozen assets. During review, a holder with
    ///      any record in a reviewed or redeem-frozen currency must wait; otherwise the entire
    ///      requested burn must fit within healthy records. Qualified exits keep their full burn
    ///      and use only those records, without redistributing the reviewed currency's exposure.
    function payableBasket(IReserveManager reserves, address holder, uint256 requested)
        public
        view
        returns (MintRedeemController.Basket memory basket)
    {
        IReserveManager.ReserveAssetView[] memory rows = reserves.assetRecords();
        uint256 n = rows.length;
        basket.tokens = new address[](n);
        basket.scale = new uint256[](n);
        basket.payableValue = new uint256[](n);
        bool anyArm = reserves.openArmCount() != 0;
        uint256 unavailableRecords;
        for (uint256 i; i < n; ++i) {
            IReserveManager.ReserveAssetView memory row = rows[i];
            basket.tokens[i] = row.asset;
            basket.scale[i] = row.scale;
            basket.grossPayable += row.units * row.scale;
            bool pending = anyArm && reserves.assetAdjudicationPending(row.asset);
            if (pending || row.frozenRedeem) unavailableRecords |= uint256(1) << i;
            if (pending) {
                if (basket.pendingAsset == address(0)) basket.pendingAsset = row.asset;
                continue;
            }
            basket.payableValue[i] = row.payableValue;
            basket.totalPayable += row.payableValue;
        }
        if (basket.pendingAsset == address(0)) return basket;
        basket.recordUnits = new uint256[](n);
        basket.totalPayable = 0;
        for (uint256 i; i < n; ++i) {
            (uint256 units,, uint256 reserved,) = reserves.recordOf(holder, basket.tokens[i]);
            if (units != 0 && (unavailableRecords & (uint256(1) << i)) != 0) return basket;
            basket.recordUnits[i] = units;
            basket.recordValue += units * basket.scale[i];
            uint256 allocatable = basket.payableValue[i] / basket.scale[i];
            allocatable = allocatable > reserved ? allocatable - reserved : 0;
            basket.payableValue[i] = allocatable * basket.scale[i];
            basket.totalPayable += basket.payableValue[i];
        }
        if (basket.recordValue == 0 || basket.recordValue < requested) return basket;
        basket.pendingAsset = address(0);
        basket.grossPayable = basket.totalPayable;
    }

    /// @notice Quotes the normal basket floor, or the holder's record allocation during review.
    /// @dev The record allocator is shared with settlement. Value includes existing same-asset
    ///      pending-claim accounting when a healthy record is not immediately funded. A requested
    ///      value that cannot be represented by the recorded currencies refuses instead of falling
    ///      through to a general basket allocation for the remainder.
    function previewSplit(MintRedeemController.Basket memory basket, uint256 valueOut)
        public
        pure
        returns (uint256[] memory amounts, uint256 valuePaid)
    {
        uint256 n = basket.tokens.length;
        if (basket.recordValue != 0) {
            valuePaid = ReserveBasketLib.shareRecord(basket.scale, basket.recordUnits, basket.recordValue, valueOut);
            if (valuePaid != valueOut) return (new uint256[](n), 0);
            amounts = basket.recordUnits;
            for (uint256 i; i < n; ++i) {
                uint256 allocatable = basket.payableValue[i] / basket.scale[i];
                if (amounts[i] > allocatable) amounts[i] = allocatable;
            }
            return (amounts, valuePaid);
        }
        amounts = new uint256[](n);
        for (uint256 i; i < n; ++i) {
            uint256 payable_ = basket.payableValue[i];
            if (payable_ == 0) continue;
            uint256 units = Math.mulDiv(valueOut, payable_, basket.totalPayable) / basket.scale[i];
            if (units == 0) continue;
            amounts[i] = units;
            valuePaid += units * basket.scale[i];
        }
    }
