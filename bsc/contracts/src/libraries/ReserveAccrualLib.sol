...
library ReserveAccrualLib {
    ...
    function configureEncoded(bytes calldata encoded) public {
        IContinuousAccrual.Modules memory m = abi.decode(encoded, (IContinuousAccrual.Modules));
    ...
    function enable() public {
        ...
        requireConsumers(m);
        uint256 deployed = ReserveStorageLib.layout().totalDeployedPrincipal;
    ...
    function setFee(uint16 feeBps, address recipient) public {
        requireFresh();
        ReserveAccrualStorageLib.State storage s = ReserveAccrualStorageLib.state();
        ...
        if (recipient != s.feeRecipient) materialize(2);
        uint64 at = ReserveAccrualStorageLib.now64();
    ...
    function prepareNativeLoss() public {
        ReserveAccrualStorageLib.State storage s = ReserveAccrualStorageLib.state();
        if (s.enabled) materialize(s.feeRecipient == s.modules.vault ? 3 : 1);
    }
    ...
    function _validateRoutes(IContinuousAccrual.Modules memory m, ReserveStorageLib.ReserveStorage storage native)
        private
        view
    {
    ...
    /// @notice Checks the native entry boundary as well as the frozen accrual window.
    /// @dev The host supplies its own reentrancy state. Authorization stays in the host.
    function requireNativeIdle(bool entered) public view {
        requireIdle();
        if (ReserveAccrualStorageLib.state().modules.token != address(0) && entered) {
            revert ReserveAccrual_OperationInProgress();
        }
    }
    ...
    function materialize(uint8 legs) public returns (uint256 senior, uint256 fee) {
        requireFresh();
    ...
    function rawBacking() internal view returns (uint256 backing, uint256 recognized) {
        ReserveStorageLib.ReserveStorage storage native = ReserveStorageLib.layout();
    ...
    function _proveDelivery(
        ...
        (uint256 backing, uint256 recognized) = rawBacking();
