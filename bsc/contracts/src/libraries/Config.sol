...
library Config {
    ...
    // -- Collateral class IDs (ADR-0037 D8: THREE live at genesis) --------
    //
    // ADR-0003 locks "all five classes at genesis" for the Ethereum instance. ADR-0037 D8
    // reopens it for THIS instance only and selects three. The ids are renumbered rather than
    // left sparse because `NUM_CLASSES` is a loop bound: it bounds a per-class iteration in the
    // curator, waterfall, default-manager, points, commitment-ledger and registry modules, and a
    // sparse numbering would make every one of those loops walk dead classes.
    //
    // Renumbering is safe because no module branches on a class id. The one exception is
    // `CLASS_FILM_TAX_CREDITS`, which `ClaimBridge` uses for the rule that a state id is present
    // for that class and zero for every other, and it keeps id 1. The marked-to-market model is
    // selected by `ClassParams.model`, never by an id, so digital assets moving from 5 to 3
    // changes nothing about how that class is treated.
    uint256 internal constant CLASS_FILM_TAX_CREDITS = 1;
    uint256 internal constant CLASS_RENEWABLE_ENERGY = 2;
    /// @dev Marked-to-market class (ADR-0015): margin-call and liquidation remedies, NOT the
    ///      receivable model of classes 1 and 2.
    uint256 internal constant CLASS_DIGITAL_ASSETS = 3;
    uint256 internal constant NUM_CLASSES = 3;

    // -- Credit-layer defaults (ADR-0004; governance-adjustable) ---------
    /// @dev USD amounts in 18 decimals (USDfr units).
    uint256 internal constant DEFAULT_FIRST_LOSS_PER_CLASS = 10_000_000e18;

    // -- Governance defaults (ADR-0037 D3(b): a timelock, no Governor) ----
    uint256 internal constant TIMELOCK_MIN_DELAY = 2 days;

    /// @notice The off-chain half of the governance path, as a floor.
    /// @dev ADR-0037 D3(b). Governance on this instance is a `TimelockController` whose sole
    ///      proposer is the operations Safe. There is no vote, so there is no voting delay and no
    ///      voting period, and the Ethereum instance's two governance timing constants have no
    ///      meaning here. The real path from "a guardian arms the custody freeze" to "governance
    ///      has executed" is: Safe coordination and signature collection, then `schedule()`, then
    ///      `getMinDelay()`, then `execute()`. Only the third term is on chain.
    ///
    ///      This constant is the floor for everything before it. It is a POLICY number about the
    ///      operations Safe's worst-case signer availability, not a derivation, and it is an
    ///      operational review item that Forest Road must sign off before genesis.
    ///
    ///      DO NOT set this to zero on the reasoning that there is no vote. The curator custody
    ///      pre-arm exists to outlast the ratification path, and removing the Governor did not
    ///      remove the human half of that path: it made it the only half that is not on chain.
    ///      Deleting the two Governor terms without replacing them would drop the derived floor
    ///      from ten days to two, cutting the protection over curator first-loss capital by five
    ///      times as a silent side effect of a governance-topology change.
    ///
    ///      The value 8 days is chosen so that `GOV_SAFE_COORDINATION_WINDOW + TIMELOCK_MIN_DELAY`
    ///      is ten days, exactly reproducing the Ethereum instance's floor, which keeps the
    ///      derived pre-arm duration at fifteen days and its cooldown at thirty. ADR-0037 D11
    ///      says to copy the timing parameters; this is the arithmetic that makes "copy" mean the
    ///      same protection rather than the same terms.
    uint256 internal constant GOV_SAFE_COORDINATION_WINDOW = 8 days;
    ...
    ///      Heartbeat/liquidity calibration is an economic-review item (ADR-0022 section X.5).
    ...
    ///      heartbeat (5000/30 ~= 167). Economic-review item (ADR-0022 section X.5).
    ...
    ///      settle until `requestedAt + DEFAULT_REDEEM_COOLDOWN`.
    ///
    ///      WHY 21 DAYS ON THIS INSTANCE. On Ethereum this value was justified by matching the
    ///      staked-governance unbonding period, so a senior and a backstop staker faced the same
    ///      exit friction. There is no staked-governance layer here (ADR-0037 D3a(ii)), so that
    ///      justification does not carry and the number needs its own: it is long enough that a
    ///      senior cannot exit ahead of a foreseeable loss, which is the front-running and
    ///      loss-dodge defence the cooldown exists for, and it is unchanged from the Ethereum
    ///      instance so the two books price exit friction identically. Economic-review item.
    ...
    ///      band. From the marked-to-market class tuple in `script/Deploy.s.sol`
