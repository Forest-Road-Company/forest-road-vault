...
library ReserveAccrualServiceLib {
    ...
    function service(uint256 id) public returns (uint256 capitalized) {
        ...
        if (work.stopped) {
            uint256 face = ReserveStorageLib.layout().deployed[id] + ReserveAccrualStorageLib.facilityUnposted(id);
