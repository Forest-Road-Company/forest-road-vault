...
/// @notice Facility NFTs use the lower half of uint256. Governance-assigned custody incidents use
///         the upper half, so the two kinds can never collide in any per-event mapping. On
///         Ethereum the mapping in question was sGROVE's event-coverage book; there is no layer
///         two on this instance (ADR-0037 D3a(ii)), and the split survives because
///         `DefaultManager` and `ReserveManager` still need custody incidents to be distinguishable
///         from facility token ids by inspection alone.
