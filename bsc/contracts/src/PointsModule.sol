...
///         each accruing `B x integral m(tau-S) dtau x rate(tau) x mult / (1 day x unit)`:
///         - sUSDfr shares (mult 1x, also earns the vault yield),
...
///         `m` is the 1.0x->2.0x maturity ramp (patience favored). Accrual is permissionless
///         per wallet and sybil-safe (linear in balance; a fresh wallet resets the ramp).
///
///         WHAT A POINT IS, AND WHAT IT IS NOT (C-11, ADR-0037 D9 - REWRITTEN DELIBERATELY, DO
///         NOT RESTORE THE ETHEREUM WORDING). The Ethereum instance's header said points "may
///         inform a future GROVE allocation". There is no GROVE on this instance and no token for
///         points to inform (ADR-0037 D3): copying that sentence forward would have described an
///         entitlement that does not exist here, in the one file a reader opens to find out what a
///         point is. A point is a NON-TRANSFERABLE RECORD OF PARTICIPATION OVER TIME and nothing
///         else. It confers no claim on the protocol, no distribution, no governance weight and no
///         redemption right, and this contract implements no mechanism by which it could.
///
///         WHETHER POINTS MAY EVER BE GIVEN A FORWARD-LOOKING MEANING IS A QUESTION FOR COUNSEL,
///         NOT FOR THIS COMMENT. ADR-0037 D9 records that operating Points on a second instance is
///         a counsel re-review item under ADR-0016. Nothing here characterises any instrument, and
///         nothing here should be read as doing so. Any wording that attaches an expectation to a
///         point is counsel's to write and Forest Road's to publish.
///
///         ECONOMIC-INTEGRITY HARDENING (why the ledger has to be honest regardless):
...
contract PointsModule is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint32 internal constant MAX_MULTIPLIER_BPS = 200_000; // 20x ceiling (bounds accrual product)

    uint8 internal constant KIND_SHARES = 0; // mult 1x
    ...
    ///      `RedemptionQueue.Request` did (the Ethereum tree's other precedent, `SGrove.Unbond`,
    ///      does not exist on this instance). Array elements are laid out
    ...
    /// @notice Initializes with the genesis rate epoch (1 point/unit/day, USDfr 3x, curator 5x).
    ...
    ///      each epoch contributes `B x (F(b-S) - F(a-S)) x rate_epoch / (1 day x unit)`,
    ...
    /// @dev F(x) = integral0x m(tau)dtau, m(tau) = 1 + min(tau, RAMP)/RAMP, WAD-scaled.
