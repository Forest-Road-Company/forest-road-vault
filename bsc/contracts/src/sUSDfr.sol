...
import {VaultFeeMath} from "./libraries/VaultFeeMath.sol";
...
import {VaultAccrualLib} from "./libraries/VaultAccrualLib.sol";
import {Config} from "./libraries/Config.sol";
import {PointsHookGas} from "./libraries/PointsHookGas.sol";
import {Roles} from "./libraries/Roles.sol";

/// @title sUSDfr - the yield-bearing vault (ADR-0005)
/// @notice ERC-4626 vault over USDfr. Once bound, `totalAssets()` includes physical USDfr
///         and the vault's unissued continuous-interest claims. Neutral token delivery leaves
///         that ownership unchanged. Legacy mode subtracts any optional unvested stream.
///         Value leaves only via the loss
    ...
    IsUSDfr
{
    ...
    /// @notice Continuous accrual uses immediate recognition and cannot overlap yield vesting.
    error SUSDfr_AccrualVestingConflict();
    ...
    /// @param compliance Compliance registry. It does NOT gate vault entry: `deposit` and `mint`
    ///        apply no allowlist check, so staking is open to any USDfr holder. The registry is
    ///        used here for exactly two things, the sanctions screen `canTransfer` applies to
    ///        SHARE transfers, and the protocol-exempt set the fee recipient must belong to. The
    ///        allowlist gates the reserve-asset boundary, meaning minting and redeeming USDfr
    ///        against reserve assets, and nothing else. The previous wording said this registry
    ///        gated vault entry, which is the opposite of what the code does; it is corrected
    ///        rather than deleted because a reader relying on it would draw a false conclusion
    ///        about who may stake.
    ...
    function totalAssets() public view override(ERC4626Upgradeable, IERC4626) returns (uint256) {
        return _pricedAssets(0);
    }

    /// @notice Permanently binds this vault to its configured continuous accounting reserve.
    /// @dev No historical income is invented by binding. Reserve activation and loan admission
    ///      independently establish the accrual start after all modules have been wired. Bind
    ///      USDfr to this reserve first; the vault verifies that binding and the controller route.
    function setAccrualReserve(address reserve) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        if (_storage().yieldVestingPeriod != 0 || unvestedYield() != 0) revert SUSDfr_AccrualVestingConflict();
        _accrueFees();
        VaultAccrualLib.bind(reserve, asset());
    }

    /// @notice The continuous reserve, or zero before governed binding.
    function accrualReserve() external view returns (address) {
        return address(VaultAccrualLib.state().reserve);
    }

    /// @notice Coherent prices and permission for a neutral delivery without a guarded write callback.
    function accrualPricingState() external view returns (IContinuousAccrual.PricingState memory) {
        return _accrualPricingState();
    }
    ...
    function feeExchangeRate() public view returns (uint256) {
        return _feeExchangeRateAtSupply(VaultAccrualLib.feeSupply(totalSupply()), Math.Rounding.Floor);
    }
    ...
    ///      - `currentExchangeRate()` is the section 1.3 monotonicity subject and must keep tracking
    ...
    ///      always, hence redemption NAV <= deposit NAV always - the invariant ADR-0022 section Y.2 owes.
    ...
    ///      holds by construction and the queue can never distribute above its budget (section 1.3).
    ...
    function maxWithdraw(address owner) public view override(ERC4626Upgradeable, IERC4626) returns (uint256) {
        if (paused() || !VaultAccrualLib.available()) return 0;
    ...
    function maxDeposit(address receiver) public view override(ERC4626Upgradeable, IERC4626) returns (uint256) {
        return (paused() || !VaultAccrualLib.available() || _isDegenerate()) ? 0 : super.maxDeposit(receiver);
    }
    ...
    function maxMint(address receiver) public view override(ERC4626Upgradeable, IERC4626) returns (uint256) {
        return (paused() || !VaultAccrualLib.available() || _isDegenerate()) ? 0 : super.maxMint(receiver);
    }
    ...
    function previewDeposit(uint256 assets) public view override(ERC4626Upgradeable, IERC4626) returns (uint256) {
        return Math.mulDiv(
            ...
            VaultAccrualLib.entryAssets(asset()) + 1,
            Math.Rounding.Floor
        );
    }
    ...
    function setImpairmentSource(address source) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        VaultAccrualLib.validateImpairmentSource(source);
    ...
    function clearUnreadableImpairmentSource() external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        ...
        bytes32 failureHash = VaultAccrualLib.impairmentRecoveryFailure(source);
    ...
    ///      cascade (a section 1.3 monotonicity break), short-changing any queued senior settling next
    ...
    ///      parameter-assertion script (CLAUDE.md section 2.1) would trip exactly that. The early
    ...
    function setYieldVestingPeriod(uint64 period) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        if (period != 0 && address(VaultAccrualLib.state().reserve) != address(0)) {
            revert SUSDfr_AccrualVestingConflict();
        }
        ...
        // the crystallized stream is a state transition in its own right (CLAUDE.md section 3.1):
    ...
    function _accrueFees() private returns (uint256 managementShares, uint256 performanceShares) {
        ...
        VaultFeeMath.Calculation memory calc = _calculateFees($);
        $.lastFeeAccrual = uint64(block.timestamp);
        ...
        // rolls back.
        if (totalFeeShares != 0) {
            if (address(VaultAccrualLib.state().reserve) != address(0)) {
                IContinuousAccrual.PricingState memory pricing = _accrualPricingState();
                pricing.feeAdjustedShares = totalSupply() + totalFeeShares;
                VaultAccrualLib.begin(pricing, pricing.feeAdjustedShares);
            }
    ...
    /// @dev Retains the historical read order; linked arithmetic preserves the deployed size bound.
    function _calculateFees(SUSDfrStorage storage $) private view returns (VaultFeeMath.Calculation memory calc) {
        uint64 last = $.lastFeeAccrual;
        calc.elapsed = last == 0 ? 0 : block.timestamp - uint256(last);
        uint256 supply = totalSupply();
        if (supply == 0) return calc;
        VaultFeeMath.Inputs memory inputs;
        inputs.supply = supply;
        inputs.markedAssets = redemptionTotalAssets();
        inputs.performanceMarkedAssets = _performanceFeeTotalAssets();
        inputs.highWaterMark = $.highWaterMark;
        inputs.elapsed = calc.elapsed;
        inputs.virtualShares = 10 ** _decimalsOffset();
        inputs.shareUnit = 10 ** decimals();
        inputs.managementFeeBps = $.managementFeeBps;
        inputs.performanceFeeBps = $.performanceFeeBps;
        return VaultFeeMath.calculate(inputs);
    }
    ...
    function _feeAdjustedSupply() private view returns (uint256 supply) {
        (bool cached, uint256 value) = VaultAccrualLib.cachedPrice(4);
    ...
    function _performanceFeeTotalAssets() private view returns (uint256) {
        return _pricedAssets(3);
    }

    /// @dev Shared linked path retains the existing mark ordering and callback snapshot.
    function _pricedAssets(uint8 kind) private view returns (uint256) {
        return VaultAccrualLib.pricedAssets(asset(), unvestedYield(), address(_storage().impairmentSource), kind);
    }

    /// @dev Computes all parts of a price ratio before opening a delivery/share-operation window.
    function _accrualPricingState() private view returns (IContinuousAccrual.PricingState memory p) {
        SUSDfrStorage storage $ = _storage();
        bool allowed = !$.feeAccrualInProgress && !$.shareUpdateInProgress && $.feeOperationKind == FEE_OPERATION_NONE
            && (!_reentrancyGuardEntered() || VaultAccrualLib.state().materializationRequested);
        return VaultAccrualLib.capture(
            asset(), unvestedYield(), address($.impairmentSource), _feeAdjustedSupply(), allowed
        );
    }

    /// @dev Legacy/off mode preserves its existing callback behavior until governed binding.
    function _beginAccrualPricing() private {
        if (address(VaultAccrualLib.state().reserve) != address(0)) {
            VaultAccrualLib.begin(_accrualPricingState(), totalSupply());
        }
    }
    ...
    /// @dev LEGACY/OFF-MODE AUDIT NOTE (R6-I2): OZ `_withdraw` burns shares before transferring assets, so
    ...
    ///      inflated window. A bound continuous-accrual vault freezes complete price ratios
    ///      across both balance effects, including the separate gross fee-rate denominator.
    function _update(address from, address to, uint256 value) internal override {
        if (from != address(0) && to != address(0)) VaultAccrualLib.requireIdle();
        SUSDfrStorage storage $ = _storage();
    ...
    /// @dev A callback cannot leave new share allowances behind during a frozen operation.
    function _approve(address owner, address spender, uint256 value, bool emitEvent) internal override {
        VaultAccrualLib.requireIdle();
        super._approve(owner, spender, value, emitEvent);
    }

    /// @dev Governance membership remains stable throughout a frozen share or delivery operation.
    function _grantRole(bytes32 role, address account) internal override returns (bool) {
        VaultAccrualLib.requireIdle();
        return super._grantRole(role, account);
    }

    /// @dev Includes both governed revocation and holder-initiated renunciation.
    ...
    function _authorizeUpgrade(address) internal view override onlyRole(Roles.UPGRADER_ROLE) {
        ...
        VaultAccrualLib.validateImpairmentSource(address(_storage().impairmentSource));
    }
