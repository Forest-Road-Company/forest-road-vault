...
import {ITimelockSchedule} from "./interfaces/IGovernanceSchedule.sol";
...
/// @notice Cascade LAYER 1 (CLAUDE.md section 1.3 ordering): curator capital absorbs realized
///         losses before any depositor impairment. ADR-0037 D3a(ii): there is NO layer-two
///         backstop on this instance, so curator first-loss is the WHOLE junior tranche and
///         what it declines reaches senior principal directly. Posting first-loss is not a gate.
    ...
    ICuratorModule
{
    ...
    struct CuratorStorage {
        ...
        // executing - handing a loss that curator capital is LAYER 1 for straight to senior
        // depositors (there is no layer two on this instance, ADR-0037 D3a(ii)).
        // `reserves` is the source of the derived freeze predicate; `governanceSchedule` supplies
        // the live governance-path length the guardian pre-arm must outlast.
        //
        // RENAMED FROM `governor` (C-06). There is no Governor on this instance: governance is a
        // `TimelockController` whose sole proposer is the operations Safe, so the only live timing
        // source is that timelock's own `getMinDelay()`. The field is named for what it holds.
        IReserveManager reserveManager;
        address governanceSchedule;
    ...
    /// @dev AUDIT FIX (F3-PA-d). Gas ceiling on each governance-parameter read. The governance
    ///      schedule address is governance-set and its code is not ours; an unbounded staticcall
    ///      lets a hostile or broken timelock burn the caller's gas and take `preArmCustodyFreeze`
    ///      down with it. An out-of-gas sub-call returns `success == false` here, which falls back
    ///      to the `Config` floor - the guardian still arms.
    ...
    function postFirstLoss(uint256 classId, uint256 amount) external nonReentrant whenNotPaused {
        ...
        // that residual is curator capital, it is layer 1, and CLAUDE.md section 1.3 requires it
        // to absorb before depositor principal. A set-aside stops absorbing, so the next loss of
        // that size would skip layer 1 and land straight on SENIOR PRINCIPAL - there is no layer
        // two here to catch it (ADR-0037 D3a(ii)) - and because the residual is NOT bounded as
        // dust (see above), the amount skipped is not bounded either.
    ...
    function withdrawFirstLoss(uint256 classId, uint256 amount) external nonReentrant whenNotPaused {
        ...
        // `absorbGlobalLoss` charges every pool pro-rata, so every pool is at risk.
        // See `IReserveManager.custodyLossUnabsorbed` for the limbs and the window each
        ...
        returns (uint256 absorbed, uint256 residual)
    {
        if (loss == 0) revert Curator_ZeroAmount();
        CuratorStorage storage $ = _storage();
        // DYNAMIC, sized from `Config.NUM_CLASSES`. The Ethereum tree used a fixed `uint256[5]`,
        // a second place the class count was written down; Solidity will not accept a library
        // constant as a fixed-array length, so a fixed array here would have to hard-code 3 and
        // could silently disagree with `Config` (C-10).
        uint256[] memory balances = new uint256[](Config.NUM_CLASSES);
        uint256[] memory allocations = new uint256[](Config.NUM_CLASSES);
        uint256 total;
    ...
    ///      arm - including the one-transaction case `setGovernanceSchedule(address(0))`, which
    ///      collapses the 90-day window to the 15-day `Config` floor - a good-faith re-arm SHORTENS the
    ...
    ///
    ///      The on-chain term is governance-mutable, so the live reading (when a governance
    ///      schedule is wired and readable) is taken and used whenever it is longer than the
    ///      `Config` launch floor. The +50% is the margin over the protocol-enforced minimum:
    ///      proposal drafting, signature collection, queueing and execution latency all sit on top
    ///      of it.
    ///
    ...
    ///      silently absent.
    ///
    ///      THE SHAPE PROBE (ADR-0037 D1). The Ethereum module probed `usdc()`, the single
    ///      immutable reserve asset. There is no single reserve asset here: the reserve is a
    ///      governed registry of two or more assets, so the probe is `reserveAssets()` and it must
    ///      come back NON-EMPTY. That both requires code and rejects an address that is not a
    ///      ReserveManager, and it additionally refuses to wire a reserve with no listed asset -
    ///      a reserve that could not pay a basket leg is not a reserve this module should be
    ///      deriving a custody-freeze predicate from.
    function setReserveManager(address reserves_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if (reserves_ != address(0)) {
            ...
            try IReserveManager(reserves_).reserveAssets() returns (address[] memory listed) {
                if (listed.length == 0) revert Curator_InvalidReserveManager(reserves_);
            } catch {
    ...
    /// @inheritdoc ICuratorModule
    /// @dev AUDIT FIX (R6-CF1), renamed by C-06. Zero leaves the `Config` launch parameters as the
    ///      sole source of the pre-arm duration - safe, because those are a FLOOR the live reading
    ///      can only raise. A non-zero address must have code; whether it actually ANSWERS is a
    ///      separate question that `governanceScheduleReadable()` publishes, because an
    ///      address-equality check alone would pass against an address that answers nothing
    ///      (C-06b).
    function setGovernanceSchedule(address schedule) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (schedule != address(0) && schedule.code.length == 0) revert Curator_InvalidGovernanceSchedule(schedule);
        _storage().governanceSchedule = schedule;
        emit GovernanceScheduleUpdated(schedule);
    }
    ...
    /// @notice The wired governance timelock the pre-arm duration is derived from (0 = `Config`
    ///         floor only).
    /// @return The wired `TimelockController`, or the zero address when explicitly unwired.
    function governanceSchedule() external view returns (address) {
        return _storage().governanceSchedule;
    }

    /// @inheritdoc ICuratorModule
    /// @dev C-06b - THIS EXISTS TO CLOSE A SILENT HOLE, NOT AS DECORATION. On Ethereum a mis-wired
    ///      governance source was caught by `Validate.s.sol` asserting `governor() == d.governor`
    ///      PLUS the ERC-6372 clock assertions. Here the clock check is gone, so address equality
    ///      alone would pass against an address that answers nothing, and the module would fall
    ///      back to the `Config` floor with nothing on chain saying the live reading had gone
    ///      dark. `custodyPreArmCoversLiveGovernancePath()` cannot report it either: it compares
    ///      the duration against the DERIVED path, so it reads true in exactly that state.
    ///      `Validate.s.sol` requires this.
    function governanceScheduleReadable() external view returns (bool) {
        return _liveGovernancePath(_storage().governanceSchedule) != 0;
    }
    ...
    ///
    ///      BSC (C-06, ADR-0037 D3(b)) - READ THIS BEFORE CHANGING EITHER TERM. On Ethereum the
    ///      floor was `GOV_VOTING_DELAY (1 day) + GOV_VOTING_PERIOD (7 days) + TIMELOCK_MIN_DELAY
    ///      (2 days) = 10 days`, giving a 15-day pre-arm and a 30-day cooldown. There is no
    ///      Governor here, so there is no voting delay and no voting period - but DELETING those
    ///      two terms without replacing them would collapse the floor to 2 days, the duration to 3
    ///      and the cooldown to 6: a FIVEFOLD reduction in the protection over curator first-loss
    ///      capital, delivered silently as a side effect of a governance-topology change. The
    ///      human half of the ratification path did not disappear with the Governor; it became the
    ///      only half that is not on chain. `Config.GOV_SAFE_COORDINATION_WINDOW` is that half,
    ///      and it is a Forest Road policy number, not an engineering default.
    function _governancePath() private view returns (uint256) {
        uint256 path = Config.GOV_SAFE_COORDINATION_WINDOW + Config.TIMELOCK_MIN_DELAY;
        uint256 live = _liveGovernancePath(_storage().governanceSchedule);
    ...
    /// @dev AUDIT FIX (R6-CF1), as reduced for this instance by C-06. The LIVE half of the
    ///      governance path, in SECONDS, or zero when it cannot be read.
    ///
    ///      BSC (ADR-0037 D3(b)): the live reading is the TIMELOCK's `getMinDelay()` and nothing
    ///      else, because there is no Governor and no vote. `IGovernanceSchedule`'s ERC-6372 clock
    ///      check is deleted with it - a `TimelockController`'s delay is denominated in seconds by
    ///      its own definition, so there is no clock to disagree with, and probing for a
    ///      `CLOCK_MODE()` a `TimelockController` does not implement would make the live reading
    ///      permanently dark. Every other F3-PA-d defence is RETAINED VERBATIM (gas bound,
    ///      32-byte returndata check, per-term cap, no `try`/`catch`): the address is
    ///      governance-set and its code is not ours.
    ///
    ///      THE ASYMMETRY ON THE LAST LINE IS DELIBERATE AND LOAD-BEARING. The live reading ADDS
    ///      the off-chain Safe window to the live `minDelay`, so a governance `updateDelay(30
    ///      days)` genuinely LENGTHENS the derived pre-arm (the R6-CF1 property), while an
    ///      `updateDelay(0)` can never shorten it below the `Config` floor, because the caller
    ///      takes the maximum of the two. Returning the bare `minDelay` here would let governance
    ///      shorten the protection over layer-1 capital in one transaction.
    ///
    ///      Zero is safe: the caller keeps the `Config` floor.
    ///
    ///      AUDIT FIX (F3-PA-d) - WHY THERE IS NO `try`/`catch` HERE ANY MORE, AND WHY IT MUST NOT
    ///      COME BACK. R6-CF1 read the governance parameters through `try`/`catch` and asserted in
    ///      its own comment that the leading `code.length` check made that safe. IT DOES NOT.
    ///      `code.length` only proves the address is a contract; it says nothing about the
    ///      callee's ANSWER. An address with a permissive fallback - a proxy whose implementation
    ///      slot is empty, a Safe, anything with `fallback() external {}` - answers with SUCCESS
    ///      and EMPTY returndata, and Solidity's `try`/`catch` does NOT catch the `abi.decode`
    ///      failure that follows (measured: the decode revert escapes the inner `catch` entirely).
    ///      That reverted `custodyPreArmDuration()` and through it `preArmCustodyFreeze()` -
    ///      disarming the guardian exactly when it is needed, which is the failure mode the
    ///      original comment was written to rule out. That hazard is SHARPER here, not softer: the
    ///      wired address is the operations Safe's own timelock, and a Safe is precisely the
    ///      permissive-fallback shape described above.
    ///
    ///      The read is therefore a RAW staticcall whose returndata length is checked BEFORE any
    ...
    ///        - `GOV_READ_GAS` bounds the sub-call so a gas-burning timelock cannot take the
    ///          guardian's arm down with it.
    ///        - the term is capped before summing so a hostile parameter cannot overflow.
    /// @param timelock_ The wired governance schedule, or zero when explicitly unwired.
    /// @return The live path length in seconds, or zero when it cannot be read.
    function _liveGovernancePath(address timelock_) private view returns (uint256) {
        if (timelock_ == address(0) || timelock_.code.length == 0) return 0;
        (bool okMin, uint256 minDelay_) = _readUint(timelock_, ITimelockSchedule.getMinDelay.selector);
        if (!okMin) return 0;
        return Config.GOV_SAFE_COORDINATION_WINDOW + _capTerm(minDelay_);
    }
    ...
    ///      rather than `CUSTODY_PRE_ARM_MAX_DURATION`: capping a term at the DURATION ceiling was
    ///      what let capped terms sum past it and then be silently truncated. The path is bounded
    ///      once, as a whole, in `_derivePreArm`.
