...
///         losses BEFORE depositor principal (CLAUDE.md section 1.3 cascade ordering).
///         ADR-0037 D3a(ii): there is NO layer-two backstop on this instance, so curator
///         first-loss is the whole of the junior tranche and what it declines reaches senior
///         principal directly. Posting first-loss is not a gate - until it is posted, every
///         credit loss reaches senior principal with nothing in front of it.
///         Multi-curator stakes in one class share losses exactly
...
interface ICuratorModule {
    ...
    // `round` disambiguates share attribution across a wipe-out (audit fix, section 3.1).
    ...
    /// @notice AUDIT FIX (R6-CF1), renamed by C-06. The governance timelock the pre-arm duration
    ///         is derived from (0 = the `Config` launch parameters are used as the floor and
    ///         nothing overrides them).
    /// @dev ADR-0037 D3(b): this is a `TimelockController`, not a Governor. Only its
    ///      `getMinDelay()` is read.
    /// @param schedule The newly wired governance schedule, or zero.
    event GovernanceScheduleUpdated(address indexed schedule);
    ...
    /// @notice AUDIT FIX (R6-CF1), renamed by C-06. `setGovernanceSchedule` was given an address
    ///         with no code, which could never supply a live governance path.
    /// @param schedule The rejected address.
    error Curator_InvalidGovernanceSchedule(address schedule);
    ...
    /// @return residual Loss remaining after this layer. On this instance there is no next
    ///         JUNIOR layer (ADR-0037 D3a(ii)), so it is senior loss.
    function absorbLoss(uint256 classId, uint256 loss) external returns (uint256 absorbed, uint256 residual);

    /// @notice Absorbs a classless protocol loss across every class pool, weighted by balances
    ...
    ///         live guardian pre-arm. Class-independent: a custody loss is allocated across EVERY
    ///         pool by `absorbGlobalLoss`, so the freeze is protocol-wide.
    ...
    ///         governance path, with the `Config` launch parameters as a floor, so it can never
    ///         expire before governance could possibly have ratified it.
    /// @dev BSC (C-06): the path is `Config.GOV_SAFE_COORDINATION_WINDOW` - the off-chain Safe
    ///      coordination and signature-collection leg - plus the timelock's `getMinDelay()`. There
    ///      is no vote to outlast, but the human half of the ratification path remains and is now
    ///      the only half that is not on chain.
    ...
    /// @notice AUDIT FIX (R6-CF1), renamed by C-06. Wires the governance timelock the pre-arm
    ///         duration is derived from. Zero leaves the `Config` launch parameters standing as
    ///         the sole source.
    /// @param schedule The `TimelockController` to read `getMinDelay()` from, or zero to unwire.
    function setGovernanceSchedule(address schedule) external;

    /// @notice C-06b. True when the live timelock reading is actually being taken, as opposed to
    ///         the module silently falling back to the `Config` floor because the schedule is
    ///         unwired or unreadable.
    /// @dev `custodyPreArmCoversLiveGovernancePath()` compares the duration to the DERIVED path,
    ///      so it reads true even when the live source is dead. `script/Validate.s.sol` requires
    ///      this one as well, because on this instance an address-equality check alone would pass
    ///      against an address that answers nothing.
    /// @return True when the live `getMinDelay()` reading is being taken, false when the module is
    ///         silently on the `Config` floor.
