...
interface IRevisionedImpairmentSource is IImpairmentSource {
    ...
    ///      That pairing is REQUIRED even on an instance with no layer two: it is a fail-closed
    ///      conjunction and dropping a limb of one is how a guard becomes decoration (C-08).
    ...
    /// @dev NULL OBJECT on this instance (ADR-0037 D3a(ii)) - `DefaultManager` hard-returns zero
    ///      because there is no layer two. The consumer's `current >= snapshot` check therefore
    ///      reads `0 >= 0`, which is the correct answer and not a missing check.
