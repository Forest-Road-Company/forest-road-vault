...
/// @title IWaterfallEngine - repayment routing (brief Part 5 section 9)
/// @notice Routes every attested repayment with exact conservation (CLAUDE.md section 1.3):
...
interface IWaterfallEngine {
    ...
    /// @dev No cash moved. `balanceAfter` and `periodEnd` let the register reconstruct a facility's
    ///      whole capitalisation history from logs alone, and `rateBps` is the SNAPSHOT the period
    ///      ran at rather than the facility's live rate, which is what makes amendments
    ///      forward-only. See `WaterfallEngine.capitalizePik`.
    /// @param tokenId The facility.
    /// @param classId Its collateral class.
    /// @param amount 18-decimal value capitalised.
    /// @param balanceAfter The facility's deployed principal after the act.
    /// @param periodEnd The interval boundary this capitalisation settled.
    /// @param rateBps The rate the settled interval ran at.
    ...
    ///      split is reconstructable from logs alone (CLAUDE.md section 3.1).
    ...
    ///      reconstructable from logs alone as `fee + withheld` (CLAUDE.md section 3.1). DO NOT "simplify"
    ...
    /// @notice A distribution named a facility that has never been bound to a reserve asset, so
    ///         there is no currency in which the receipt can be denominated.
    /// @dev BSC instance (ADR-0037 D1): the reserve custodies more than one stablecoin and binds a
    ///      facility to exactly one of them on its first credit-side act. A facility with no
    ///      binding has never been funded, so a repayment against it is nonsense rather than a
    ///      rounding question, and it fails closed here rather than reverting deeper with a
    ///      reserve-internal error.
    error Waterfall_FacilityAssetUnbound(uint256 tokenId);
    ...
    /// @notice The facility is flagged past due, so it may not capitalise. Past due is a
    ///         `DefaultManager` flag rather than a `ClaimBridge` state, so this is a separate check.
    ...
    /// @notice The facility has never been funded, so its PIK clock has not started.
    ...
    /// @notice The interval's interest rounds to nothing on this asset's scale grid.
    ...
    /// @dev BSC instance (ADR-0037 D1): the funding CURRENCY is named by the servicer rather than
    ///      implied by a single reserve token. The named asset is what the reserve binds the
    ///      facility to, so every later repayment must arrive in the same currency. `assetAmount`
    ///      is in that asset's NATIVE units; the engine normalises it through the asset's recorded
    ///      per-asset scale and requires the result to equal the originated principal exactly.
    /// @param tokenId The Pending facility to fund.
    /// @param asset The listed reserve asset the facility is funded and bound in.
    /// @param assetAmount Native units of `asset` to deploy, inclusive of the origination fee.
    function fund(uint256 tokenId, address asset, uint256 assetAmount) external;
