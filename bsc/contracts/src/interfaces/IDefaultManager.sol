...
/// @title IDefaultManager - default handling and the two-layer loss cascade
...
///         (ADR-0012): curator first-loss then sUSDfr vault principal, in that order, never
///         skipping or inverting a layer (CLAUDE.md section 1.3).
///
///         ADR-0037 D3a(ii): THERE IS NO LAYER TWO ON THIS INSTANCE. No sGROVE, no GROVE, no
///         backstop and no setter that could install one. The layer-two fields kept in the shapes
///         below are permanent literal zeroes, so every loss log states in machine-readable form
///         that no second layer absorbed. Posting first-loss is not a gate: until curator capital
///         is posted, every credit loss reaches senior principal directly.
...
interface IDefaultManager is IRevisionedImpairmentSource, IReserveLossAbsorber {
    ...
    /// @notice The cascade record: loss == curatorAbsorbed + backstopCovered + depositorLoss,
    ///         burned in the same transaction as the write-down.
    /// @dev `backstopCovered` is LAYER 2 AND IS PERMANENTLY ZERO on this instance (ADR-0037
    ///      D3a(ii)). The field is retained rather than removed so the off-chain register decoder
    ///      stays shared with the Ethereum instance and so the absence of a second layer is
    ///      auditable from the chain, not only from a document.
    event LossRealized(
    ...
    ///           3. `realizeLoss` (`_reduceDefaulted`) - the H-2 re-anchor clamp, for any
    ///              remainder above the principal still outstanding after the write-down.
    ...
    /// @dev Advances on every declared-default, past-due, recovery and realization transition
    ///      that can change the conservative impairment state. The Ethereum instance also advanced
    ///      it on a backstop-wiring change; there is no backstop to wire here.
    ...
    ///         consumed layer-2 coverage (the latter is unreachable here: ADR-0037 D3a(ii)).
    ///         Replacing the ledger would erase per-event state.
    ...
    /// @param required What the controller asked junior capital to fund.
    /// @param curatorAbsorbed Layer 1 - curator first-loss, pro-rata over the standing pools. It
    ///        is the WHOLE junior draw on this instance.
    /// @param backstopCovered Layer 2 - PERMANENTLY ZERO (ADR-0037 D3a(ii)). Retained as an
    ///        on-chain statement that no second layer absorbed.
    ...
    ///      returns what curator first-loss could actually provide, which may be zero. The drawn
    ...
    /// @return drawn USDfr actually transferred here by layer 1 (`<= required`).
    ...
    /// @notice Layer-two coverage consumed by one facility default.
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO. Retained so the observability
    ///      surface is identical across the two instances.
    function coverageConsumedByDefault(uint256 tokenId) external view returns (uint256 consumed);
    /// @notice Aggregate layer-two coverage consumed by live declared defaults.
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO.
    ...
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO. The live gross principal the
    ///      conservative walk consumes is `ICommitmentLedger.remainingPrincipalAggregate()`, a
    ///      different quantity that is deliberately not reported under a layer-two name.
    ...
    /// @dev Governance-only and one-way. It is permitted only before any live declared principal
    ///      exists, because creating a fresh ledger afterwards would erase live principal records
    ///      used for conservative redemption pricing.
    ...
    /// @notice The wired cascade backstop.
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY `address(0)`. There is no `setBackstop`
    ///      on this instance and no storage slot behind this. It is retained because
    ///      `Validate.s.sol` asserts it: a validator that says NOTHING about layer two would pass
    ///      a stack in which one had quietly been installed.
    ...
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO. Nothing can draw layer two here,
    ///      so the drawn cohort is empty by construction.
    ...
    ///         after junior capital (curator first-loss per class, and nothing after it on this
    ///         instance) absorbs - the conservative mark deducted from redemption NAV (ADR-0022).
    ...
    /// @inheritdoc IRevisionedImpairmentSource
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO. `AssessedImpairmentSource` still
    ///      snapshots it and still re-checks `current >= snapshot`; that limb now reads `0 >= 0`,
    ///      which is correct, and deleting one limb of a fail-closed conjunction is how a guard
    ///      becomes decoration (C-08).
