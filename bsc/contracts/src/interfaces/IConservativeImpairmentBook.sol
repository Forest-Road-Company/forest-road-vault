...
interface IConservativeImpairmentBook {
    ...
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY ZERO on this instance: nothing can draw
    ///      layer two, so the drawn cohort is empty by construction.
    function drawnDefaultPrincipal(uint256 classId) external view returns (uint256);

    /// @notice Layer-two coverage already drawn by defaults that are still declared-but-unrealized.
    ...
    /// @notice The cascade backstop.
    /// @dev NULL OBJECT (ADR-0037 D3a(ii)) - PERMANENTLY `address(0)`. There is no layer two and
    ///      no setter that could install one. Retained on the seam so `Validate.s.sol` can ASSERT
    ///      it: a validator silent about layer two would pass a stack in which one had quietly
    ///      been installed.
